// Level 11: TypeScript Fundamentals - Interactive Script

class Level11Manager {
    constructor() {
        this.storageKey = 'level11-progress';
    ...
    compileTypeScript(tsCode) {
        try {
            // Use the TypeScript compiler from CDN
    ...
    checkExercise(exerciseNumber) {
        ...
        const hasContent = tsContent.trim().length > 200;
        const hasTypes = tsContent.includes('interface') || tsContent.includes('type') || tsContent.includes('enum');
        const hasFunctions = tsContent.includes('function') || tsContent.includes('=>');
        
        if (hasContent && hasTypes && hasFunctions) {
            ...
            this.showNotification('Please add TypeScript code with types, interfaces, and functions to complete this exercise.', 'warning');
        }
    }
    ...
    completeLevel() {
        ...
        this.showNotification('🎉 Level 11 completed! You are now a TypeScript Master!', 'success');
        this.createConfetti();
    }

    syncWithMainHub() {
        const mainHubProgress = JSON.parse(localStorage.getItem('frontend-mastery-progress') || '{}');
        if (mainHubProgress.levels && mainHubProgress.levels[11]) {
            mainHubProgress.levels[11].completed = this.progress.completed;
            mainHubProgress.levels[11].levelCompleted = this.progress.completed >= this.progress.total;
    ...
    createConfetti() {
        const colors = ['#667eea', '#764ba2', '#48bb78', '#38a169', '#4facfe', '#00f2fe'];
...
function switchTab(tabId) {
    level11Manager.switchTab(tabId);
}

function checkExercise(exerciseNumber) {
    level11Manager.checkExercise(exerciseNumber);
}

function resetProgress() {
    level11Manager.resetProgress();
}

// Initialize
document.addEventListener('DOMContentLoaded', function() {
    window.level11Manager = new Level11Manager();
    level11Manager.switchTab('ts1');
    
    console.log('Level 11: TypeScript Fundamentals loaded!');
    console.log('TypeScript compiler is available at: window.ts');
});
